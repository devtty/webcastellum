...
public final class Base64Utils {
    ...
    public static String encode(final byte[] data) {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            switch (state) {
                case 1:
                    output[0] = B64CHARS[ic >>> 2];
                    restbits = ic & 0x03;
                    break;
                case 2:
                    output[1] = B64CHARS[(restbits << 4) | (ic >>> 4)];
                    restbits = ic & 0x0F;
                    break;
                case 3:
                    output[2] = B64CHARS[(restbits << 2) | (ic >>> 6)];
        ...
        for (i = 0; i < encoded.length(); i++) {
            byte c;
            {
                ...
                if ((alpha >= 'A') && (alpha <= 'Z')) {
                    ...
                    c = (byte) (52 + (alpha - '0'));
                } else if (alpha == (URL_SAFE ? '-' : '+')) {
                    c = 62;
                } else if (alpha == (URL_SAFE ? '_' : '/')) {
                    c = 63;
                } else if (alpha == (URL_SAFE ? '.' : '=')) {
                    break; // end
                } else {
