...
public final class HoneylinkUtils {
    ...
    public static final short nextTagPartCounterTarget(Random random) {
        if (random == null) {
            random = HONEY_RANDOM; // in public methods random can be null, meaning that a non-user-seeded random should be used
        }
    ...
    public static String generateHoneylink(Random random, final String prefix, final String suffix, final boolean isWithinTable) {
        ...
        insertDeveloperComment(random, honeylink);
        ...
        insertLink(random, honeylink, prefix, suffix);
        if (random.nextInt(10) > 8) {
            honeylink.append("\n");
        }
        if (random.nextInt(10) > 8) {
    ...
    public static boolean isHoneylinkFilename(String filename) {
        ...
        if (!Character.isUpperCase(filename.charAt(0))) {
            return false; // da der erste Buchstabe immer Uppercase ist
        }
    ...
    private static String generateTitle(final Random random) {
    ...
    static final String generateFilename(final Random random) {
        ...
        for (int i = 0; i < wordCount; i++) {
            ...
            // dedupe
            do {
                part = WORDS.get(random.nextInt(WORDS.size()));
    ...
    private static String generateDeveloper(final Random random) {
        ...
        for (int i = 0; i < 2; i++) {
            developer.append(FREQUENT_LETTERS[random.nextInt(FREQUENT_LETTERS.length)]);
        }
    ...
    private static String generateDate(final Random random) {
        ...
        if (date.get(Calendar.MONTH) == Calendar.DECEMBER && date.get(Calendar.DAY_OF_MONTH) > 20) {
    ...
    private static String generateParameterName(final Random random, final boolean hex) {
        if (hex) {
            return PARAMETER_PARTS_HEX[random.nextInt(PARAMETER_PARTS_HEX.length)];
        }
    ...
    private static void insertLink(Random random, final StringBuilder honeylink, final String prefix, final String suffix) {
        ...
        if (prefix != null) {
            honeylink.append(prefix);
        }
        honeylink.append(generateFilename(random));
        if (suffix != null) {
            honeylink.append(suffix);
        }
    ...
    private static void insertDeveloperComment(Random random, final StringBuilder honeylink) {
